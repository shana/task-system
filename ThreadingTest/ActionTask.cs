...
namespace GitHub.Unity
{
    ...
    class FuncListTask<T, TData> : ListTaskBase<T, TData>
    {
        protected Func<T> Callback { get; }
        protected Func<T> CallbackIfDependentFailed { get; }

        public FuncListTask(CancellationToken token, Func<T> action)
            : base(token)
        {
            Guard.ArgumentNotNull(token, "token");
            Guard.ArgumentNotNull(action, "action");
        ...
        public FuncListTask(CancellationToken token, Func<T> action, ITask dependsOn)
            : base(token, dependsOn)
        {
            Guard.ArgumentNotNull(token, "token");
            Guard.ArgumentNotNull(action, "action");
        ...
        public FuncListTask(CancellationToken token, Func<T> action, Func<T> actionIfDependentFailed, ITask dependsOn)
            : base(token, dependsOn)
        {
            Guard.ArgumentNotNull(token, "token");
            Guard.ArgumentNotNull(action, "action");
